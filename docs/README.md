# $HEART Documentation

**The Autonomous Blockchain — Born from AI. Evolved by AI. For AI.**

Welcome to the official documentation for the $HEART blockchain, built by [Humans.ai](https://agents.humans.ai).

---

## Quick Links

| Document | Description |
|----------|-------------|
| [Overview](./overview.md) | What is $HEART, the thesis, and positioning |
| [Architecture](./architecture.md) | Technical architecture, modules, tokens, chain config |
| [User Guide](./user-guide.md) | Creating wallets, spawning entities, earning compute |
| [Validator Guide](./validator-guide.md) | Running a validator node on the $HEART network |
| [Developer Guide](./developer-guide.md) | Building on $HEART with CosmJS, REST, and gRPC |
| [API Reference](./api-reference.md) | Complete transaction and query endpoint reference |
| [Tokenomics](./tokenomics.md) | $HEART and Compute Token economics |
| [Governance](./governance.md) | Proposals, voting, and self-evolution |

---

## Testnet Information

| Resource | Endpoint |
|----------|----------|
| **RPC** | `http://5.161.47.118:26657` |
| **REST / LCD** | `http://5.161.47.118:1317` |
| **gRPC** | `5.161.47.118:9090` |
| **Faucet** | `http://5.161.47.118:4500` |
| **EVM JSON-RPC** | `http://5.161.47.118:8545` |
| **Chain ID** | `heart-testnet-1` |
| **Denomination** | `uheart` (1 HEART = 1,000,000 uheart) |
| **Explorer** | [agents.humans.ai/explorer](https://agents.humans.ai/explorer) |

---

## Getting Started

1. **Users** — Start with the [User Guide](./user-guide.md) to create a wallet and spawn your first AI Human.
2. **Validators** — Follow the [Validator Guide](./validator-guide.md) to set up a node and join the network.
3. **Developers** — Read the [Developer Guide](./developer-guide.md) and [API Reference](./api-reference.md) to build on $HEART.

---

## Source Code

- **Chain**: [heart](https://github.com/humansai/heart) — Cosmos SDK chain with identity, existence, compute, and migration modules
- **App**: [humans-agi-base](https://github.com/humansai/humans-agi-base) — Next.js frontend with AI Human management

---

*Built by Humans.ai — agents.humans.ai*
